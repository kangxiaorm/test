...
class GridRegion(Region):
    ...
    def __init__(self,id_='',bounds=[],**kwargs):
        bs=tuple(bounds)
        left,bottom,right,top=bs
        boundary=[(left,bottom),(right,bottom),(right,top),(left,top),(left,bottom)]
...
class PartitionByGrid(object):
    ...
    def partition_region(self,bounds,width=100,height=100):
        ...
        self.colume_count=math.floor(x_times)+1
        print("end partition preprocess")
        print("total row:{0} colume:{1}".format(self.row_count,self.colume_count))
        self.grids={}
...
#            
#            for col in range(0,self.colume_count):
#                print("generating grid row:{0} colume:{1}".format(row,col))
#                row_index=row+1
#                col_index=col+1
...
#                self.grids[grid.id]=grid
    ...
    def get_grid_id(self,row,colume):
        return "{0},{1}".format(row,colume)
    
    def get_grid_by_index(self,row:int,colume:int):
        
        id_=self.get_grid_id(int(row),int(colume))
        if id_ not in self.grids:
            l=self.left+colume*self.grid_width
            b=self.bottom+row*self.grid_height
            r=l+self.grid_width
            t=b+self.grid_height
            self.grids[id_]=GridRegion(id_,(l,b,r,t),type_=self.type_,row=row,colume=colume)
...
#        points=[(left,top),(right,top),(right,bottom),(left,bottom),(left,top)]
        grid=GridRegion(id_=self.get_grid_id(row,col),bounds=(left,bottom,right,top),type_=self.type_,row=row,colume=col)
        self.grids[grid.id]=grid
        return grid
