...
def train_epoch(model, optimizer, baseline, lr_scheduler, epoch, val_dataset, problem, tb_logger, opts):
    ...
    # Generate new training data for each epoch     每个epoch重新生成epoch_size条训练数据, graph_size和veh_size都是固定的, 可以修改吗？
    ...
    # save results_1114_1120 every checkpoint_epoches, saving memory
...
def train_batch(
    ...
    x = move_to(x, opts.device)     # move_to()是干什么的?
    bl_val = move_to(bl_val, opts.device) if bl_val is not None else None
    
    
    # Evaluate proposed model, get costs and log probabilities
    cost, log_likelihood, log_veh = model(x)  # both [batch_size]   # x 属于一个批量的数据
